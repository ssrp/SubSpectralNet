    ...
    for fold in folds:
        ...
        if not os.path.isfile(fold_model_filename) or overwrite:
            ...
            if param.get_path('learner.parameters.validation_set') and param.get_path('learner.parameters.validation_set.enable', True):
                ...
                validation_files = db.eval(fold=fold).unique_files#dcase_util.containers.MetaDataContainer()
            ...
            if param.get_path('learner.parameters.generator.enable'):
                ...
                import numpy as np
                np.save('X_train_right_40.npy', X_train)
                np.save('Y_train_right_40.npy', Y_train)
                np.save('X_validation_right_40.npy', X_validation)
                np.save('Y_validation_right_40.npy', Y_validation)
